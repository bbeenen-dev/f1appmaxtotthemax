    ...
    <div className="min-h-screen bg-[#0f111a] text-white p-4 md:p-8 font-f1">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <button onClick={() => router.push('/admin')} className="text-slate-500 text-xs uppercase hover:text-[#005AFF] transition-all flex items-center gap-2">
            <span>←</span> DASHBOARD
          </button>
        ...
        <div className="bg-[#161a23] p-4 rounded-2xl border border-slate-800 mb-6 shadow-inner relative">
          <label className="text-xs font-black uppercase text-slate-500 italic mb-2 block ml-1">Selecteer Grand Prix</label>
          <select 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/5 border-b border-slate-800">
                <th className="p-4 text-sm font-black uppercase text-slate-500 italic">Deelnemer</th>
                {isSprintWeekend && (
                  <th className="p-4 text-sm font-black uppercase text-[#005AFF] italic text-center">Sprint</th>
                )}
                <th className="p-4 text-sm font-black uppercase text-slate-500 italic text-center">Qualy</th>
                <th className="p-4 text-sm font-black uppercase text-slate-500 italic text-center">Race</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={isSprintWeekend ? 4 : 3} className="p-12 text-center">
                    ...
                    <span className="text-sm font-black italic uppercase text-slate-500">Gegevens laden...</span>
                  </td>
                </tr>
              ) : (
                statusList.map(user => (
                  <tr key={user.user_id} className="border-b border-slate-800/50 hover:bg-white/5 transition-colors group">
                    <td className="p-4">
                      <p className="font-black uppercase italic text-base text-white group-hover:text-[#005AFF] transition-colors">{user.user_name}</p>
                      <p className="text-[10px] text-slate-600 font-mono uppercase tracking-tighter italic">ID: {user.user_id.slice(0,8)}...</p>
                    </td>
                    {isSprintWeekend && (
        ...
        <div className="mt-6 flex justify-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase text-slate-500 italic">Ingevuld</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 bg-red-500 rounded-full"></div>
            <span className="text-xs font-bold uppercase text-slate-500 italic">Ontbreekt</span>
          </div>
        </div>
...
function StatusIcon(done: boolean) {
  return done ? (
    <div className="inline-flex items-center justify-center w-8 h-8 bg-green-500/10 border border-green-500/20 text-green-500 rounded-lg shadow-sm">
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
      </svg>
    </div>
  ) : (
    <div className="inline-flex items-center justify-center w-8 h-8 bg-red-500/10 border border-red-500/20 text-red-500 rounded-lg opacity-40">
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
